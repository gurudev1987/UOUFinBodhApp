...
import { DIGITAL_PAYMENT_TOPICS } from './index';

export default function DigitalPaymentDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const router = useRouter();

  const topic = DIGITAL_PAYMENT_TOPICS.find((t) => t.id === id);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {/* How to Use */}
        <Section title="📝 How to Use / Apply">
...
const styles = StyleSheet.create({
  ...
  banner: {
    minHeight: 220, justifyContent: 'flex-end',
    padding: 20, paddingTop: 60, overflow: 'hidden',
  },
  bannerImage: {
